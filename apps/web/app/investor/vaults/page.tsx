    ...
    <main className="min-h-screen bg-rayls-black text-white px-6 py-12 lg:px-12">
      <header className="max-w-7xl mx-auto mb-8">
        ...
        <p className="text-sm text-rayls-grey">
          Capital deployment opportunities across risk-tranched agricultural
          credit pools.
        </p>
      </header>
          ...
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-rayls-grey uppercase bg-rayls-dark border-b border-rayls-border">
              <tr>
                ...
                <th className="px-6 py-4 font-medium tracking-wider w-48">
                  Utilization
                </th>
            ...
            <tbody className="divide-y divide-rayls-border">
              {vaults.map((vault) => {
                ...
                const utilization = Math.min(
                  Math.max((activeCount / 10) * 100, 15),
                  95
                ); // Mock utilization %
                ...
                return (
                  ...
                  >
                    <td className="px-6 py-4">
                      <Link
                        href={`/investor/vaults/${encodeURIComponent(
                          vault.id
                        )}`}
                        className="block"
                      >
                        ...
                        <div className="text-xs text-rayls-grey truncate max-w-[200px]">
                          {vault.description}
                        </div>
                      </Link>
                    </td>
                    <td className="px-6 py-4">
                      <Badge
                        variant="rayls"
                        className="border-rayls-purple text-rayls-purple"
                      >
                    ...
                    <td className="px-6 py-4 text-rayls-grey">12 Months</td>
                    <td className="px-6 py-4">
